Shader "MLF/PhysicsUpdateForces_Avoid"
{
	Properties
	{
		...
		Avoid0("Avoid0", VECTOR) = (0,0,0,0)
		Avoid1("Avoid1", VECTOR) = (0,0,0,0)
		AvoidRadius("AvoidRadius", Range(0,1) ) = 0.1
		AvoidScalar("AvoidScalar", Range(-100,100) ) = 0.1
	}
	SubShader
	{
		...
		Pass
		{
			...
			float3 CalcNewForce(float3 Position,float3 SpringPosition,float3 Velocity,float3 Random)
			{
				...
				float3 AvoidForce = float3(0,0,0);
				float3 Avoids[2] = { Avoid0,Avoid1};

				for ( int i=0;	i<2;	i++ )
				{
					float3 Avoid = Avoids[i];
					float3 Delta = Position - Avoid;
					if ( length(Delta) > AvoidRadius )
						continue;

					AvoidForce += Delta * AvoidScalar;
				}
